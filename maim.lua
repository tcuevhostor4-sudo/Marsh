...
local function installUpdate()
    ...
    if isSampAvailable() then
        sampAddChatMessage('[M-AIM] Начата загрузка обновления ' .. tostring(remoteVersion) .. '...', -1)
    end
    ...
    downloadUrlToFile(SCRIPT_URL .. '?t=' .. tostring(os.time()), updateScriptPath, function(_, status)
        ...
        if not validDownloadedScript(updateScriptPath) then
            ...
            if isSampAvailable() then sampAddChatMessage('[M-AIM] Ошибка загрузки: файл повреждён.', -1) end
        ...
        if not backupOk then
            ...
            if isSampAvailable() then sampAddChatMessage('[M-AIM] Ошибка: не удалось заменить текущий файл.', -1) end
            updateMsgUntil = os.clock() + 5.0
            return
        end
        ...
        if not replaceOk then
            ...
            if isSampAvailable() then sampAddChatMessage('[M-AIM] Ошибка установки обновления.', -1) end
            updateMsgUntil = os.clock() + 5.0
            return
        end
        ...
        if isSampAvailable() then
            sampAddChatMessage('[M-AIM] Обновление загружено и установлено. Перезапуск скрипта...', -1)
        end
...
function imgui.OnDrawFrame()
    ...
    imgui.SameLine()
    if imgui.Button(buttonText, imgui.ImVec2(buttonWidth, 28)) and not updateDownloading then
        if updateAvailable then installUpdate() else checkForUpdates(true) end
    end
