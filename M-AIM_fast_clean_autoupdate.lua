...
local function validateSource(data)
    ...
    or header:find('404: not found', 1, true) then
        return false, 'GitHub вернул ошибку'
    end
    ...
    lua_thread.create(function()
        ...
        if not finished then
            ...
            if manual and isSampAvailable() then
                sampAddChatMessage('[M-AIM] Не удалось проверить обновление.', -1)
            end
            return
        end
        ...
        if not valid then
            ...
            if isSampAvailable() then
                sampAddChatMessage('[M-AIM] Ошибка обновления: ' .. tostring(reason) .. '.', -1)
            end
            return
        end
        ...
        if not writeFile(currentPath, source) then
            ...
            updateDownloading = false
            os.remove(updateScriptPath)
            sampAddChatMessage('[M-AIM] Ошибка записи обновления.', -1)
            return
        end
        ...
        if not installedOk then
            ...
            updateDownloading = false
            os.remove(updateScriptPath)
            sampAddChatMessage('[M-AIM] Старый файл восстановлен.', -1)
            return
        end
        ...
        if isSampAvailable() then
            sampAddChatMessage(
                '[M-AIM] Обновлено до версии ' .. remoteVersion ..
                '. Перезапуск...',
                -1
            )
        end
...
function main()
    ...
    -- Удаляем старые временные файлы от предыдущих версий обновления.
    os.remove(updateScriptPath)
